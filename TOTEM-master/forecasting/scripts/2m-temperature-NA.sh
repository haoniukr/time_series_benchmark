...
data_path_name=../../datasets/bigdatasets/2m-temperature-NA.csv
data_name=custom
random_seed=2021
pred_len=96
gpu=0
...
python -u forecasting/save_revin_data.py \
  ...
  --save_path "forecasting/data/2m-temperature-NA-"${year_start}
...
python forecasting/train_vqvae.py \
  --config_path forecasting/scripts/2m-temperature-NA.json \
  --model_init_num_gpus $gpu \
  --data_init_cpu_or_gpu cpu \
  --comet_log \
  --comet_tag pipeline \
  --comet_name vqvae_weather \
  --save_path "forecasting/saved_models/2m-temperature-NA-"${year_start}"/"\
  --base_path "forecasting/data/2m-temperature-NA-"${year_start}\
  --batchsize 4096
done
...
root_path_name=/mnt/data1/python/time_series_benchmark/TOTEM-master/
data_path_name=../../datasets/bigdatasets/2m-temperature-NA.csv
model_id_name=2m-temperature-NA
data_name=custom
seq_len=336
...
python -u forecasting/extract_forecasting_data.py \
  ...
  --enc_in 126 \
  --gpu $gpu\
  --save_path "forecasting/data/2m-temperature-NA-"${year_start}"/Tin"$seq_len"_Tout"$pred_len"/"\
  --trained_vqvae_model_path "forecasting/saved_models/2m-temperature-NA-"${year_start}"/CD64_CW256_CF4_BS4096_ITR15000/checkpoints/final_model.pth"\
