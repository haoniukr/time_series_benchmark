...
data_path_name=../../datasets/pems-bay.csv
data_name=custom
random_seed=2021
pred_len=96
gpu=0
...
python -u forecasting/save_revin_data.py \
  ...
  --gpu $gpu\
  --batch_size 512 \
  --save_path "forecasting/data/pems-bay"


gpu=0
python forecasting/train_vqvae.py \
  --config_path forecasting/scripts/pems-bay.json \
  --model_init_num_gpus $gpu \
  --data_init_cpu_or_gpu cpu \
  --comet_log \
  --comet_tag pipeline \
  --comet_name vqvae_weather \
  --save_path "forecasting/saved_models/pems-bay/"\
  --base_path "forecasting/data/pems-bay"\
  --batchsize 4096
...
python -u forecasting/extract_forecasting_data.py \
  ...
  --enc_in 325 \
  --gpu $gpu\
  --save_path "forecasting/data/pems-bay/Tin"$seq_len"_Tout"$pred_len"/"\
  --trained_vqvae_model_path "forecasting/saved_models/pems-bay/CD64_CW256_CF4_BS4096_ITR15000/checkpoints/final_model.pth"\
